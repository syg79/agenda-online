            ...
            <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 z-40 relative shadow-sm">
                ...
                <div className="flex bg-slate-100 p-1 rounded-lg">
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 text-[11px] font-bold rounded-md transition-all ${activeTab === 'waiting' ? 'bg-white text-amber-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Realizado ({tabCounts.waiting})
                    </button>
                </div>
                ...
                <DashboardSidebar
                    ...
                    items={(() => {
                        ...
                        if (globalSearch) {
                            ...
                            else if (activeTab === 'waiting') baseItems = data?.schedule.filter((s: Booking) => s.status === 'REALIZADO' || s.status === 'COMPLETED') || [];
                        }
                        return baseItems;
                    })()}
                                ...
                                {locationConflicts.length > 0 && (
                                    <div className="bg-orange-50 border-l-4 border-orange-500 p-3 rounded-r-lg shadow-sm">
                                        ...
                                        <div className="text-[11px] text-orange-700 leading-tight">
                                            ...
                                            Verifique se um único profissional pode realizar o atendimento consolidado para otimizar custos e logística.
                                        </div>
                                    </div>
                                )}
                ...
                {selectedSlot && (
                    <div className="w-96 bg-white border-l border-slate-200 shadow-xl overflow-y-auto z-40 animate-in slide-in-from-right duration-300 flex flex-col relative">
                        <SlotSuggestionList
                            ...
                            onSelectOrder={handleSelectOpportunityFromSlot}
