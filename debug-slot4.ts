...
async function debug() {
    ...
    const dateStr = '2026-02-23'; const p_centro = pending.find(p => p.address.includes('Sete de Abril') || p.address.includes('XV de Novembro') || p.address.includes('Marechal Deodoro') || p.address.includes('Victor Ferreira')); console.log("FOUND CENTRO PIN:", p_centro?.id, p_centro?.clientName, p_centro?.address, p_centro?.latitude, p_centro?.longitude);
    const timeStr = '10:00';
